...
\chapter{Zad. 5}
Regulator PID zosta³ przetestowany poprzez zmianê wartoœci zadanej z punktu pracy do $ \ang{50}C $ a nastêpnie do $ \ang{30}C $. Parametry regulatora wynosi³y $ K = 6 $, $ T_{i} = 65 $, $ T_{d} = \num{1.25} $ a wyniki eksperymentu z nim widaæ na rysunku ~\ref{zad5_pid}. Regulator DMC o parametrze $ \lambda = 5 $ zosta³ przetestowany dla skoku wartoœci zadanej do $ \ang{37}C $, co widaæ na rysunku ~\ref{zad5_dmc}. Oba regulatory co prawda osi¹gnê³y wartoœci zadane , jednak nieliniowy charakter obiektu utrudni³ im zadanie. Rozwi¹zaniem tego problemu mo¿e byæ zast¹pienie regulatorów tradycyjnych rozmytymi.
\begin{figure}[b]
	\centering

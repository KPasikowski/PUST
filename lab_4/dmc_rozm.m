...
for i = 1:Kk
    ...
    if(u_mf <= 46)
        du = ke1*(yzad(i)-y(i)) - ku1*duph;
    end
    if(u_mf > 46 && u_mf < 54)
        du = (fprzynaleznosci(u_mf,par1)*(ke1*(yzad(i)-y(i)) - ku1*duph)+fprzynaleznosci(u_mf,par2)*(ke2*(yzad(i)-y(i)) - ku2*duph))/(fprzynaleznosci(u_mf,par1)+fprzynaleznosci(u_mf,par2));
    end
    if(u_mf >= 54)
        du = ke2*(yzad(i)-y(i)) - ku2*duph;
    end
